...
<div class="grid simple">
    ...
    <form action="{$url.admin}/create_new_item" method="post" class="validate">
        {$csrf_token}
        <div class="grid-body">
            {$form_alert}
            <div class="row">
                        ...
                        <select id="item_author" class="form-control" name="item_author" required>
                            ...
                            {if $list_users}
                                {foreach from=$list_users item=$list_user}
                                    <option value="{$list_user->user_id}">{$list_user->user_firstname} {$list_user->user_lastname} ({$list_user->user_username})</option>
                                {/foreach}
                            {/if}
                        </select>
                    </div>
                                ...
                                {foreach from=$upl_subs item=$sub}
                                    ...
                                    <optgroup label="{$sub->sub_cat_name}">
                                        {foreach from=$sub->child_cats item=chi}
                                            <option value="{$chi->child_cat_id}">{$chi->child_cat_name}</option>
                                        {/foreach}
                                    </optgroup>
                                    {/if}
                                {/foreach}
