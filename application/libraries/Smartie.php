...
class Smartie extends Smarty {
    ...
    function __construct()
    {
        ...
        $CI =& get_instance();
        $CI->load->helper('url');
        $CI->load->library('themes');
        $active_theme = $CI->themes->getTheme();

        if($CI->uri->segment(1) == 'admin')
        {
            $this->template_dir = APPPATH . "views/templates/admin";
        }
        else
        {
            $this->template_dir = APPPATH . "views/templates/".$active_theme;
        }
